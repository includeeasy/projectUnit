...
import org.jboss.forge.BasePackageMarker;
...
public class TestPluginTest  extends SingletonAbstractShellTest{
	...
	@Test
	public void testRun() {		
		if(!getShell().getCurrentDirectory().getChild("hello").exists()){
			System.err.println("Project not exist,we will create it!");
			getShell().execute("mkdir hello");
			getShell().execute("cd hello");
			queueInputLines("y");
			getShell().execute("new-project --named first --topLevelPackage net,flowas.tdddemo");
			getShell().execute("project install-facet flowas.mock");			
		}else{

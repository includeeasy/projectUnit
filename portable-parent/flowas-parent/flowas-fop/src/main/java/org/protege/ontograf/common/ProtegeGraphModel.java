...
public class ProtegeGraphModel extends DefaultGraphModel {
	...
	protected GraphNode addNode(OWLEntity entity) {
		return addNode(entity, owlModelManager.getRendering(entity), owlEditorKit.getOWLWorkspace().getOWLIconProvider().getIcon(entity), getNodeType(entity));
	}
	...
	private Set<GraphArc> findOutgoingConditionsRelationships(OWLEntity entityOfInterest, boolean mustBeVisible) {
		...
		convertOWLClassExpressionsToArcs(owlClass, owlClass.getEquivalentClasses(owlOntology), arcs, owlEditorKit.getOWLWorkspace().getOWLIconProvider().getIcon(owlClass), mustBeVisible);
